...
/**
 * 
 * Resuelve le problema del  cálculo de los caminos con aristas disjuntas mediante una red de flujo
 * 
 * @author Miguel Toro
 *
 */

public class CaminosAristasDisjuntas {
	...
	public static void creaFichero(String fileIn, String fileOut, Ciudad from, Ciudad to){
		...
		SimpleDirectedGraph<Ciudad,Carretera> gs = Graphs2.toDirectedGraph((SimpleGraph<Ciudad,Carretera>) graph);
		PrintWriter f = null;
		try {
		...
		for(Ciudad c:gs.vertexSet()){
			if (c.equals(from)) {
				...
				f.println(c + ",Intermediate,0.0,inf,0.0,"+c); //No hay limitaciones para que pasen caminos
				                                 //por cada vértice
			}
		}		
		f.println("#EDGE#");
	...
	public static void main(String[] args) {
		...
		CaminosAristasDisjuntas.creaFichero("ficheros/andalucia.txt","ficheros/andaluciaFlow2.txt",from,to);
		
		FlowGraphSolution fs = SolveFlowGraph.solve(TipoDeOptimizacion.Max, "ficheros/andaluciaFlow2.txt");

    	FlowGraphSolution fs2 = FlowGraphSolution.createOnlySaturated(fs);
		fs.getGraph().exportToDot("ficheros/andaluciaFlowGrafo2.gv");
		fs2.exportToDot("ficheros/andaluciaFlowSoluciones2.gv");
		Strings2.toFile(fs.getGraph().getConstraints(), "ficheros/andaluciaFlowConstraints2.txt");
