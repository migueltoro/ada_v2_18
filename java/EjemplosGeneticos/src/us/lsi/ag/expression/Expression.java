...
import us.lsi.tiposrecursivos.NaryOperator;
import us.lsi.tiposrecursivos.Exp;
import us.lsi.tiposrecursivos.Operator;
import us.lsi.tiposrecursivos.OperatorCode;
import us.lsi.tiposrecursivos.UnaryOperator;
...
public class Expression implements ExpressionProblemAG<Exp<Double>, Double> {
	...
	public List<Operator> getOperators() {		
		OperatorCode<BiFunction<Double, Double, Double>> plus = Operator.getBinary("+", ExpType.Double, ExpType.Double);
		OperatorCode<BiFunction<Double, Double, Double>> multiply = Operator.getBinary("*", ExpType.Double, ExpType.Double);
		UnaryOperator<Double,Double>  sqrt = Operator.getUnary("sqrt", ExpType.Double);
		UnaryOperator<Double,Double> pot2 = Operator.getUnary("^2", ExpType.Double);
		UnaryOperator<Double,Double> pot3 = Operator.getUnary("^3", ExpType.Double);
	...
	public Double fitnessFunction(ExpressionChromosome<Double> cr) {
		...
		for(int i=0;i<lsx.size();i++){
			...
			r2 = cr.getExp().getValue()-lsv.get(i);
			r = r + r2*r2;			
		}
		return -r;
	}
	...
	@Override
	public NaryOperator<Double,Double> getAccumulator() {
		return Operator.getNary("+", ExpType.Double);
	}
