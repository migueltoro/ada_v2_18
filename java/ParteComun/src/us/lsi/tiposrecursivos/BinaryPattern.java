...
public sealed interface  BinaryPattern<E> permits PTree<E>, PEmpty<E>, PLeaf<E>, PVarLabel<E>, PVarTree<E> {
	...
	public static <E> BinaryPattern<E> of(BinaryTree<String> tree, Function<String,E> f){
		...
		case BLeaf<String> t && t.label().startsWith("_")-> BinaryPattern.varLabel(t.label());
		case BLeaf<String> t -> BinaryPattern.leaf(f.apply(t.label()));
		case BTree<String> t && t.label().startsWith("_") -> BinaryPattern.varTree(t.label(),
	...
	public static <E> Matches<E> match(BinaryPattern<E> pt, BinaryTree<E> tree) {
		return switch(pt) {
		case PEmpty<E> p  && tree instanceof BEmpty<E> t -> Matches.of(true);
		case PLeaf<E> p && tree instanceof BLeaf<E> t -> Matches.of(p.label().equals(t.label())); 
		case PVarLabel<E> p -> Matches.ofTrees(new HashMap<>(Map.of(p.varName(), tree))); 
		case PTree<E> p && tree instanceof BTree<E> t && p.label().equals(t.label()) -> {
			Matches<E> r = match(p.left(), t.left());
			if(r.match)  r.add(match(p.right(), t.right()));
			yield r;
		}
		case PVarTree<E> p && tree instanceof BTree<E> t -> {
