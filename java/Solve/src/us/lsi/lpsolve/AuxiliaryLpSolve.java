...
public class AuxiliaryLpSolve {
	 ...
	 * @param f Una función que devuelve u n String
	...
	public static String sum_1(Integer n, String x, 
		...
		return IntStream.range(0, n).boxed()
				.filter(p)
				.map(i -> String.format("%s %s",f.apply(i),var_1(x,i)))
				.collect(Collectors.joining(" + "));
	}
	 ...
	 * @param j Un valor constante del segundo &iacute;ndice
	 * @param x El identificador de la variable 
	 * @param f Un funci&oacute; que calcula el coeficiente del factor
	...
	public static String sum_2_1_f(Integer n, Integer j, String x, 
		...
		return IntStream.range(0, n).boxed()
				.map(i -> String.format("%s %s", f.apply(i,j),var_2(x,i,j)))
				.collect(Collectors.joining(" + "));
	}
	 ...
	 * @param x El identificador de la variable 
	 * @param pd Un bipredicado
	 * @param f Un funci&oacute; que calcula el coeficiente del f
	...
	public static String sum_2_2_f(Integer n, Integer i, String x,
			BiFunction<Integer,Integer,String> f) {
		return IntStream.range(0, n).boxed()
				.map(j -> String.format("%s %s", f.apply(i,j),var_2(x,i,j)))
				.collect(Collectors.joining(" + "));
	}
	...
	public static String sum_2(Integer n, Integer m, String id, 
		...
		return Streams2.allPairs(n, m)
				.filter(p->pd.test(p.first,p.second))
				.map(p -> String.format("%s %s",f.apply(p.first,p.second),var_2(id,p.first,p.second)))
				.collect(Collectors.joining(" + "));
	}
	 ...
	 * @param fc Una funci&oacute; que calcula una restricci&oacute;n
	 * @return \( \forall_{i=0|pd(i)}^{n-1} fc(i) \)
	 */
	public static String forAll_1(Integer n, String name,
	 ...
	 * @param pd Un predicado
	 * @param fc Una funci&oacute; que calcula una restricci&oacute;n
	 * @return \( \forall_{i=n1|pd(i)}^{n2-1} fc(i) \)
	 */
	public static String forAll_1_r(Integer n1, Integer n2, String name,
			Predicate<Integer> p, 
			Function<Integer,String> fc) {
	 ...
	 * @param pd Un predicado
	 * @param fc Una funci&oacute; que calcula una restricci&oacute;n
	 * @return \( \forall_{i=0,j=0}^{n-1,m-1} fc(i,j) \)
	 */
	public static String forAll_2(Integer n, Integer m, String name,
			BiFunction<Integer,Integer,String> fc) {
		return Streams2.allPairs(n, m)
				.map(p -> String.format("   %s%d: %s",name,p.first+m*p.second,fc.apply(p.first,p.second)))
				.collect(Collectors.joining("\n","\n","\n"));
	}
	 ...
	 * @param pd Un predicado
	 * @param fc Una funci&oacute; que calcula una restricci&oacute;n
	 * @return \( \forall_{i=0,j=0|pd(i,j)}^{n-1,m-1} fc(i,j) \)
	 */
	public static String forAll_2(Integer n, Integer m, String name,
			BiPredicate<Integer,Integer> pd, 
			BiFunction<Integer,Integer,String> fc) {
	 ...
	 * @param pd Un predicado
	 * @param fc Una funci&oacute; que calcula una restricci&oacute;n
	 * @return \( \forall_{i=n1,j=m1|pd(i,j)}^{n2-1,m2-1} fc(i,j) \)
	 */
	public static String forAll_2_r(Integer n1, Integer n2, Integer m1, Integer m2, String name,
		...
		return Streams2.allPairs(n1, n2, m1, m2)
				.filter(p->pd.test(p.first,p.second))
				.map(p -> String.format("   %s%d: %s",name,p.first+(m2-m1)*p.second,fc.apply(p.first,p.second)))
				.collect(Collectors.joining("\n","\n","\n"));
	}
	 ...
	 * @param pd Un predicado
	 * @param fc Una funci&oacute; que calcula una restricci&oacute;n
	 * @return \( \forall_{i=0,j=0, k=0|pd(i,j,j)}^{n-1,m-1,r-1} fc(i,j,k) \)
	 */
	public static String forAll_3(Integer n, Integer m, Integer r, String name, 
			TriPredicate<Integer,Integer,Integer> pd, 
			TriFunction<Integer,Integer,Integer,String> fc) {
		return Streams2.allPairs(n, m, r)
				.filter(p->pd.test(p.first,p.second,p.third))
				.map(p -> String.format("   %s%d: %s",name,p.first+m*p.second+m*r*p.third,fc.apply(p.first,p.second,p.third)))
				.collect(Collectors.joining("\n","\n","\n"));
	}
